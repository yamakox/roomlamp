"""Storage list and detail views for PVC, PV, and StorageClass."""
...
def sort_storage(items: list[StorageSummary], column: int, ascending: bool) -> list[StorageSummary]:
    """Sort storage rows. ``column`` is an index into the kind's columns."""

    def key(item: StorageSummary) -> tuple[object, ...]:
        keys = item.sort_keys
        if not keys:
...
class StorageListScreen(NavigationMixin, Screen[None]):
    BINDINGS = [
        MENU_BINDING,
        CONTEXT_BINDING,
        HOME_BINDING,
    ...
    def __init__(
        ...
        self._items: dict[str, StorageSummary] = {}
        self.sort_column = 0
    ...
    def on_mount(self) -> None:
        ...
        if self.enable_watch:
            self.run_worker(self._load_initial, exclusive=True, group='storage-load')
        else:
            self._load_sync()
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield Vertical(
            status_widget('storage-status'),
            ResourceTable(id='storage', cursor_type='row'),
            id='storage-wrap',
        )
        yield Footer()
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        ...
        if key:
            self.run_worker(self._open_detail(key), exclusive=True, group='storage-detail')
    ...
    def action_delete(self) -> None:
        key = selected_row_key(self.query_one('#storage', DataTable))
        if not key:
            return
        namespace, name = split_storage_key(self.kind, key)
        request_delete(
            self,
            self.cluster,
            self.kind,
    ...
    def _fetch(self) -> tuple[list[str], list[StorageSummary]]:
        namespaces: list[str] = []
        ...
        items = self.cluster.list_storage(self.kind, self.namespace)
        return namespaces, items

    def _apply_list(self, namespaces: list[str], items: list[StorageSummary]) -> None:
    ...
    def _apply_event(self, event_type: str, item: StorageSummary) -> None:
        self._items = apply_watch_event(self._items, event_type, item)
        self._render_table()

    def _render_table(self) -> None:
        table = self.query_one('#storage', DataTable)
        columns = STORAGE_SPECS[self.kind].columns
        if not table.columns:
            table.add_columns(*columns)
        table.clear()
        for item in sort_storage(list(self._items.values()), self.sort_column, self.sort_ascending):
            table.add_row(*item.cells, key=item.key)

    async def _open_detail(self, key: str) -> None:
        namespace, name = split_storage_key(self.kind, key)
        try:
            detail = await asyncio.to_thread(self.cluster.get_storage, self.kind, namespace, name)
        except Exception as exc:
            self._set_status(api_error_message(exc))
            return
        await self.app.push_screen(StorageDetailScreen(detail, self.cluster))

    def _start_watch(self) -> None:
        if not hasattr(self.cluster, 'watch_storage'):
            return
        self._stop_watch()
    ...
    def _set_status(self, message: str) -> None:
        set_status(self.query_one('#storage-status', Static), message)

    def _set_subtitle(self) -> None:
        context = self.info.context_name or 'no context'
        label = STORAGE_LABELS[self.kind]
        if not is_namespaced(self.kind):
...
class StorageDetailScreen(NavigationMixin, Screen[None]):
    BINDINGS = [
        MENU_BINDING,
        CONTEXT_BINDING,
        HOME_BINDING,
    ...
    def __init__(self, detail: StorageDetail, cluster: Any) -> None:
        super().__init__()
        self.detail = detail
        self.cluster = cluster
    ...
    async def _reload_detail(self) -> None:
        ...
        self._set_subtitle()
        self.query_one('#storage-detail', Static).update(_detail_text(self.detail))

    async def action_show_yaml(self) -> None:
        try:
            text = await asyncio.to_thread(
                self.cluster.get_storage_yaml,
                self.detail.kind,
                self.detail.namespace,
                self.detail.name,
            )
        ...
        await self.app.push_screen(
            YamlViewScreen(
                title,
                text,
                reload=lambda: self.cluster.get_storage_yaml(kind, namespace, name),
...
def _detail_text(detail: StorageDetail) -> str:
